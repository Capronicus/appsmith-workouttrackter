export default {
  ...
  const normalized = muscle
    ...
    .join(' ');
  return "https://duffs.homene.st/fitness/fullbody/" + map[normalized];
},
...
getOverlaysForExercise() {
  ...
  return GetExerciseMuscles.data
	...
	getOverlaysForWorkoutIntensity() {

  if (!GetWorkoutMuscles.data) {
    return [];
  }

  return GetWorkoutMuscles.data.map(row => ({
    url: this.getOverlay(row.name),
    intensity: row.intensity
  }));

},
	
getOverlaysForWorkoutExercise() {

  if (!GetWorkoutExerciseMuscles.data) {
    return [];
  }

  return GetWorkoutExerciseMuscles.data.map(row => ({
    url: this.getOverlay(row.name),
    intensity: row.intensity
  }));
