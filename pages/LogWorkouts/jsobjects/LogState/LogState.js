...
for (const ex of exercises) {
  ...
  // ⬇ OUTSIDE the set loop, INSIDE the exercise loop — once per exercise
...
// After the for loops, before this.reset():

// Clear all the per-input store keys
const keysToCleanup = [];
for (const key of Object.keys(appsmith.store)) {
  if (key.startsWith('reps_') || key.startsWith('weight_') || 
      key.startsWith('feel_') || key.startsWith('notes_') ||
      key.startsWith('inserting')) {
    keysToCleanup.push(key);
  }
}
